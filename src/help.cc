...
. ftr VBI CBI
.\}
.TH "MANAGEBACKUPS" "1" "March 2023" "managebackups 1.7.0" ""
.hy
.SH NAME
.PP
...
specified (\f[B]-p\f[R]) profile to the targetDir.
The copy is placed directly in targetDir with no year, month or day or
subdirectories.
The entire contents of targetDir are deleted before the copy begins.
...
Microsoft\[cq]s OneDrive, etc) which intentionally do not follow
symlinks and don\[cq]t respect hardlinks.
By replicating to a directory in one of your cloud synced services, you
can gain off-site replication of a single backup.
Because the cloud providers ignore hardlinks, storing your entire backup
collection there would negate all the storage benefits of hardlinks.
.TP
