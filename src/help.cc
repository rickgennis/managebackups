...
. ftr VBI CBI
.\}
.TH "MANAGEBACKUPS" "1" "March 2023" "managebackups 1.8a" ""
.hy
.SH NAME
.PP
...
.RE
.PP
When specifying a previous backup the full backupname isn\[cq]t
required; only enough to uniquely identify it.
...
.fi
.TP
\f[B]\[en]maptaghold\f[R] [\f[I]data\f[R]]
{FB} Associate a tag with a specific hold time.
Once associated, any time the tag is applied to a bag the associated
