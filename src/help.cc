...
. ftr VBI CBI
.\}
.TH "MANAGEBACKUPS" "1" "March 2023" "managebackups 1.6.8" ""
.hy
.SH NAME
.PP
...
.TP
\f[B]\[en]archive\f[R]
Set the profile to Archive mode so no pruning takes place and its
excluded from -a and -A runs.
Use with \[en]save.
...
.nf
\f[C]
Identical output to above as this is a synonym for diffing the most recent backup to its immediate predecessor.  Additionally, if \[dq]laptop\[dq] is made the default profile (such as via \[dq]**managebackups -p laptop --default --save**\[dq]) the above command can be further simplified to \[dq]**managebackups --last**\[dq].
\f[R]
.fi
.PP
