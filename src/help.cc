...
void showHelp(enum helpType kind) {
    switch (kind) {
        ...
        case hOptions: {
            ...
            + "   --replicateto [des] Replicate the latest backup to des (local, overwrite).\n\n"
            ...
            + "   --default           Make the specified profile the default.\n"
            ...
            + "   --rmn [x]           Remove the x newest (latest) backups.\n"
            + "   --rmo [x]           Remove the x oldest backups.\n"
...
. ftr VBI CBI
.\}
.TH "MANAGEBACKUPS" "1" "March 2023" "managebackups 2.3a" ""
.hy
.SH NAME
.PP
...
.TP
\f[B]\[en]interactive\f[R]
Launch the initial setup interactive wizard to assist creating a
faub-backup config from scratch.
.TP
...
It can be combined with \f[B]-p\f[R] to limit output to a single profile
or \f[B]\[en]format\f[R] to change how numbers are displayed.
Note: Profiles containing the `path' directive are excluded.
.TP
\f[B]-1\f[R]
...
Note: Profiles containing the `path' directive are excluded.
.TP
\f[B]\[en]format\f[R] \f[I][number]\f[R]
Specify an alternate number format for use with \f[B]-0\f[R],
\f[B]-1\f[R], or \f[B]\[en]analyze\f[R].
Formats 1 through 5 are supported.
.TP
\f[B]\[en]analyze\f[R] \f[I][number]\f[R]
Analyze a backup set and determine the files that change most frequently
and the files consuming the most actual disk space.
Show the top \f[I]number\f[R] files.
In MOST FREQUENTLY CHANGING FILES the percentage is how many of the
profile\[cq]s backups have a different copy of the shown file
(percentages will not sum to 100 as they\[cq]re individual).
In MOST SPACE CONSUMING CHANGES the percentage is how much of the total
backup space is consumed by the shown file (percentages would sum to 100
if all files were shown).
In both cases, the Usage number is a total for all copies (changes) of
the shown file - i.e.\ how much disk space that file is actually using
across the backup set.
.TP
\f[B]\[en]test\f[R]
Run in test mode.
...
When used with \f[B]-1\f[R] it supresses header rows and color output
(i.e.\ \f[B]\[en]nocolor\f[R]) to facilitate script processing.
.TP
...
within the profile.
It can also be the exact name of a tag if the tag is only assigned to a
single backup.
...
.TP
\f[B]\[en]replicateto\f[R] [\f[I]targetDir\f[R]]
{FB} The replicate option makes a hardlink-aware copy (i.e.\ no
appreciable additional storage) of the most recent backup of the
...
By replicating to a directory in one of your cloud synced services, you
can gain off-site replication of a single backup.
Without the replicate option, the choices would be to create a symlink
in your cloud sync directory to where your backups are housed \[en] this
would be useless because the cloud providers don\[cq]t follow links,
resulting in no backup making it to the cloud.
Or you would have to store all of your backups in the cloud sync
directory, resulting in a larger cloud bill because the provider would
ignore the hardlinks and see each copy as additional storage.
The copy of the backup in \f[I]targetDir\f[R] is not tracked and does
not appear in \f[B]-1\f[R] output.
.TP
...
errors.
.PP
\f[B]-t,\[en]tag\f[R] [\f[I]tagname\f[R]]
.TP
\f[B]-t,\[en]tag\f[R] [\f[I]tagname\f[R]:\f[I]backup\f[R]]
{FB} There are three uses of the tag directive:
.RS
.IP
.nf
\f[C]
* tag a backup as it\[aq]s taken (e.g. \[dq]mb -p main -t snapshot\[dq])
* tag a previously taken backup; syntax is tag:backup (e.g. \[dq]mb -p main -t snapshot:/var/backup/main-2024-02-23\[dq])
* display backups that match a tag (e.g. \[dq]mb -1 -t snapshot\[dq])
\f[R]
.fi
.RE
.PP
When tagging a previous backup the full backup name isn\[cq]t required;
only enough to uniquely identify it.
By contrast tag names always need to be precisely specified.
.TP
\f[B]\[en]tagrm\f[R] [\f[I]data\f[R]]
{FB} If a tag is specified it will be removed from all backups & if any
hold time is associated with it, the tag -> hold mapping is removed.
If a backup is specified, all tags will be removed from it.
Removing a specified tag from all backups can be limited in scope by
specifying \f[B]\[en]profile\f[R], though tag -> hold mapping removal is
global.
.TP
\f[B]\[en]tagls\f[R]
{FB} List all defined tags and any associated hold times.
.PP
\f[B]\[en]hold\f[R] [\f[I]holdtime\f[R]]
.TP
\f[B]\[en]hold\f[R] [\f[I]holdtime\f[R]:\f[I]backup\f[R]]
{FB} There are two uses of the hold directive:
.RS
.IP
.nf
\f[C]
* set a holdtime while a backup is taken (e.g. \[dq]mb -p main --hold 6m\[dq] for 6 months)
* set a holdtime on an existing backup (e.g. \[dq]mb -p main --hold 3w:/var/backup/main-2024-02-23\[dq])
\f[R]
.fi
.RE
.PP
When specifying a previous backup the specified name is interpreted as a
search string.
All matching backups will be updated.
\f[I]holdtime\f[R] can specify the length of time to hold a backup
(prevent deletion according to the configured
daily/weekly/monthly/yearly schedule) in any combination of days, weeks,
months or years.
Months are computed as 30 days.
Alternatively the end date of the hold can be given.
Specify 0 to remove a hold.
Use a double-colon and omit the holdtime to set a permanent hold
(e.g.\ \[lq]\[en]hold ::2024-08-21\[rq] to permanently keep all backups
with 2024-08-21 in their name).
Examples of valid \f[I]holdtime\f[R]s:
.IP
.nf
\f[C]
...
.fi
.TP
\f[B]\[en]maptaghold\f[R] [\f[I]tag:holdtime\f[R]]
{FB} Associate a tag with a specific hold time.
Once associated, any time the tag is applied to a backup, the associated
hold time will also be set.
If \f[B]\[en]maptaghold\f[R] is used to change the hold time of that tag
in the future, the previously tagged backups are not updated unless they
get re-tagged.
\f[I]holdtime\f[R] can be any of the formats supported by
\f[B]\[en]hold\f[R].
.TP
...
that one.
.TP
\f[B]\[en]ignoretouch\f[R]
{FB} Ignore changes to files compatible with the command-line `touch'
command (i.e.\ mtime changes but the content of the file is still
identical).
For example, the DHCP daemon often updates the mtime on /etc/resolv.conf
even though its contents are still the same.
This causes resolv.conf to get backed up each time and the entire backup
to persist even with \f[B]\[en]dataonly\f[R] selected, as now the backup
has as least one change.
\f[B]\[en]ignoretouch\f[R] ignores such mtime-only changes, which would
result in resolv.conf getting hardlinked on the backup server to the
previous backup\[cq]s copy, the same as if it hadn\[cq]t been
\[cq]touch\[cq]ed at all.
Note: This means that the mtime of such files (such as resolv.conf) in
that backup will show the mtime of the previous time the file was backed
up.
Such files will appear as modifications in the \f[B]-1\f[R] listing but
they won\[cq]t use any disk space or be verifiable via mtime.
This will allow the entire backup to be removed on the next run if
\f[B]\[en]dataonly\f[R] is selected and no other files had actual
content changes.
...
.SH TAGGING & HOLDING
.PP
Tags can be used to identify and track specific backups within or across
profiles.
A tag can be applied when a backup is taken or after the fact.
Tags can be specified with \f[B]-1\f[R] to see only the backups that
match a given tag or to the \f[B]\[en]diff\f[R] option to locate a
backup by tag.
Multiple backups can be given the same tag and multiple tags can be
applied to the same backup (many to many).
.PP
Holding is a method of keeping a backup after its aged out of the
configured daily/weekly/monthly/yearly quota.
A hold can be set on a backup in either relative terms (x amount of time
from when the hold is set) or absolute terms (a specific date).
Relative holds are specified as a number of days, weeks, months and/or
years.
For purposes of a hold, a month is calculated as 30 days.
.IP
.nf
\f[C]
    * 6w for 6 weeks
...
.fi
.PP
Tagging (backup organization) and holding (deletion prevention) can be
used independently.
They can also be associated together.
A given tag can be assigned a hold time (relative or absolute) such that
any time the tag is applied, the associated hold time is also applied.
See \f[B]\[en]maptaghold\f[R] for details.
.SH PERMISSIONS
.PP
