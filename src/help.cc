...
void showHelp(enum helpType kind) {
    switch (kind) {
        ...
        case hOptions: {
            ...
            + "   --compare [bkup]    Compare two arbitrary backups and list size changes\n"
...
. ftr VBI CBI
.\}
.TH "MANAGEBACKUPS" "1" "March 2023" "managebackups 1.5" ""
.hy
.SH NAME
.PP
...
.TP
\f[B]\[en]diffl\f[R] [\f[I]string\f[R]]
Similar to \f[B]\[en]diff\f[R] except display the full path (long form)
of the changed files within the context of the matching backup.
.TP
\f[B]\[en]compare\f[R] [\f[I]string\f[R]]
{FB} Compare two backups within a profile and show the type & size
change of each file.
Unlike \f[B]\[en]diff\f[R] the comparison can be with any other backup
in the profile set.
This is done by walking the backup directory structure instead of using
the cache, meaning it will be slower.
Files are displayed if they don\[cq]t share the same inodes bewteen both
backups.
...
up in a \f[B]\[en]compare\f[R].
Files are shown in the first backup, unless the given file only exists
in the second backup.
Specify \f[B]\[en]compare\f[R] twice, once to name each backup.
See \f[B]\[en]threshold\f[R] & \f[B]\[en]compfocus\f[R].
.TP
\f[B]\[en]threshold\f[R] [\f[I]limit\f[R]]
{FB} Specify a threshold to filter \f[B]\[en]compare\f[R] listings.
...
.TP
\f[B]\[en]compfocus\f[R]
{FB} Hardlink aren\[cq]t an option for directories or symlinks, meaning
directories and symlinks will show up on a \f[B]\[en]compare\f[R] even
if the data hasn\[cq]t changed.
\f[B]\[en]compfocus\f[R] omits directories and symlinks to increase the
focus on files that actually changed.
See \f[B]\[en]compare\f[R].
.TP
...
Use with -p.\ This should never be necessary.
...
.PP
\f[B]managebackups\f[R] implements several functions to inspect the
difference between individual Faub-style backups within a profile.
The \f[B]\[en]diff\f[R] and \f[B]\[en]diffl\f[R] options provide a fast
interface to list all the files that have changed between a given backup
and the chronologically previous one.
The \f[B]\[en]compare\f[R] option provides a slower interface that can
be used between any two backups in the profile set and includes more
details about what\[cq]s different (size change & file type).
.SH PERMISSIONS
.PP
...
.PP
\f[B]managebackups -0 -p laptop\f[R]
.IP
.nf
\f[C]
...
.fi
.PP
\f[B]managebackups -p laptop \[en]compare 17:2 \[en]compare 55:53\f[R]
.IP
.nf
\f[C]
