% MANAGEBACKUPS(1) managebackups 2.3a
% Rick Ennis
% March 2023
...
: Force has different functions in different contexts:
    ...
    - While executing a profile **--force** will override any existing lock and force the profile to run (backup, prune, etc).
    ...
    - With **--diff** or **--Diff** the Full Changes form of the diff will be run instead of the Cached Changes.
...
**--diff** [*backup*]
: {FB} Display the differences (files that have changed) between backups.  *backup* can be a partial string that matches any unique backup within the profile.  It can also be the exact name of a tag if the tag is only assigned to a single backup. If **--diff** is specified once, its performed on the matching backup and the chronologically previous backup.  If **--diff** is specified twice, the two given backups are diff'd.  **--force** can be used to generate the full version of the diff when **--diff** is only specified once.  Note that if a file exceeds **--maxlinks** and gets assigned a new inode (even though the file content is identical) it will also show up in a **--diff**.  See **--threshold**.  See EXAMINING BACKUPS below for full detail.
